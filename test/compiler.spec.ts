...
import { mvp, imports, nonCss, scoped, cssModules } from "./source";
...
it("works with scoped CSS", async () => {
  ...
  expect(wrapper.element.children[0].attributes.item(1)!.name).match(
    /^data\-v\-/
  );
  expect(wrapper.element.children[1].tagName).equal("INPUT");
  expect(wrapper.element.children[1].attributes.item(0)!.name).equal(
...
it("works with CSS Modules", async () => {
  ...
  if (existsSync("./test/dist/css-modules")) {
    rmSync("./test/dist/css-modules", { recursive: true });
  }
  ensureDirSync("./test/dist/css-modules");
  writeFileSync(`./test/dist/css-modules/${destFilename}`, jsCode);
